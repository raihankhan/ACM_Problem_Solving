...
int main()
{
    ...
    lli pre[n+7][4];
    pre[n][1]=pre[n][2]=pre[n][3]=0;

    rev(i, n-1, 0)
    {
        if(s[i]=='R')
            pre[i][1]=1LL+pre[i+1][1];
        else
            pre[i][1]=pre[i+1][1];
        if(s[i]=='G')
            pre[i][2]=1LL+pre[i+1][2];
        else
            pre[i][2]=pre[i+1][2];
        if(s[i]=='B')
            pre[i][3]=1LL+pre[i+1][3];
        else
            pre[i][3]=pre[i+1][3];

        //cout << pre[i][1] << " " << pre[i][2] << " " << pre[i][3] << endl;
    }
    ...
    rep(i, 0, n-2)
    {
        rep(j, i+1, n-1)
        {
            ...
            else if( (s[i]=='G' and s[j]=='B') or (s[i]=='B' and s[j]=='G') )
            {
                tot+=pre[j+1][1];
                if(j+p<n)
                {
            ...
            else if( (s[i]=='B' and s[j]=='R') or (s[i]=='R' and s[j]=='B') )
            {
                tot+=pre[j+1][2];
                if(j+p<n)
                {
