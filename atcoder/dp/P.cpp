...
lli dfs(int u,int state)
{
   // cout << u << " " << (state==0?'W':'B') << endl;
    ...
    for( auto v:g[u])
    {
        ...
        if(state==W)
        {
            tot*=(dfs(v,W)+dfs(v,B))%M;
            tot%=M;
        }
        else tot=(tot*dfs(v,W))%M;

        p++;

    }

    if(p==0) tot=max(1LL,tot);
    //cout << u << " " << (state==0?'W':'B') << tot <<  endl;

    return dp[u][state]=tot;
}
