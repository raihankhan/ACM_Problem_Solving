...
using namespace std;
lli dp[4][p5],vr[4][p5],n;

lli f(lli state , lli pos)
{
    if(dp[state][pos]) return dp[state][pos];
    if(pos==n) return vr[state][pos];
    lli p=state%3;
    lli q=(p==1)? 3 : ((p==2)? 1 : 2);
    return dp[state][pos]=vr[state][pos] + max ( f(p+1 , pos+1 ) , f( q , pos+1) ) ;
}
int main()
{
    lli ans,i,j,k,p;
    ...
    rep(i , 1 , n+1)
    {
        scln2(vr[1][i],vr[2][i]),scln(vr[3][i]);
