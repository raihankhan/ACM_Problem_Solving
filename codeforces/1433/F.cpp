...
int solve(int ro,int col,int rem,int now_sum)
{
    //error(ro,col);
    ...
    if(col==m+1 or rem==0)
    {
        return solve(ro+1,1,m/2,now_sum);
    }

    int &ret = dp[ro][col][rem][now_sum];

    if(ret!=-1) return ret;

    int res=0;

    res=solve(ro,col+1,rem,now_sum);

    if(rem)
    {
        res=max(res,arr[ro][col]+solve(ro,col+1,rem-1,(now_sum+arr[ro][col])%k));
    }

    res=max(res, solve(ro+1,1,m/2,now_sum));
