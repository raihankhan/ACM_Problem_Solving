...
inline lli solve(lli frnt,lli bck,lli tot)
{
    lli &ret=dp[frnt][bck];
    if(vis[frnt][bck]==ind) return ret;
    vis[frnt][bck]=ind;

    if(tot==m) return ret=max(arr[frnt],arr[bck]);

    lli frnttk=solve(frnt+1,bck,tot+1);
    lli bcktk=solve(frnt,bck-1,tot+1);

    if(tot<=k)
        return ret=max(frnttk,bcktk);
    else
        return ret=min(frnttk,bcktk);
