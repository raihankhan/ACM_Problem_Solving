...
void test(int T)
{
    int n,i;
    scin(n);
    veci a(n+1),b(n+1),c(n+1),p(n+1);
    rep(i , 1 , n+1) scin(a[i]);
    rep(i , 1 , n+1) scin(b[i]);
    rep(i , 1 , n+1) scin(c[i]);
    ...
    rep(i , 2 , n+1 )
    {
        ...
        if(c[i]!=p[i-1]) { p[i]=c[i]; continue; }
    }

    if(p[n]==p[1])
    {
        if(a[n]!=p[1] and a[n]!=p[n-1]) { p[n]=a[n]; }
        else if(b[n]!=p[1] and b[n]!=p[n-1]) { p[n]=b[n]; }
        else if(c[n]!=p[1] and c[n]!=p[n-1]) { p[n]=c[n]; }
    }
