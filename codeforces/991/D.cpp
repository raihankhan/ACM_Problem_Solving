...
int solve(int i,int shape)
{
    if(i>=n-1) return 0;
    if(dp[i][shape]!=-1) return dp[i][shape];
    int ways=0;
    if(shape==1)
    {
        if(s[1][i+1]=='0' and s[2][i+1]=='0' and s[2][i]=='0') ways=max(ways,1+solve(i+1,3));
    }
    else if(shape==4)
    {
        if(s[1][i+1]=='0' and s[2][i+1]=='0' and s[1][i]=='0') ways=max(ways,1+solve(i+1,2));
    }
    else if(shape==0)
    {
        if(s[1][i]=='0' and s[2][i]=='0')
        {
            if(i+1<n and s[1][i+1]=='0') ways=max(ways,1+solve(i+1,1));
            if(i+1<n and s[2][i+1]=='0') ways=max(ways,1+solve(i+1,4));
        }
        if(i+1<n and s[1][i+1]=='0' and s[2][i+1]=='0')
        {
            if(s[1][i]=='0') ways=max(ways,1+solve(i+1,2));
            if(s[2][i]=='0') ways=max(ways,1+solve(i+1,3));
        }
    }

    ways=max(ways,solve(i+1,0));
    return dp[i][shape]=ways;
}
