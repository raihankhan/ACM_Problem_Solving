...
int main()
{
    ...
    pair<int,int> chips[k+7];
    pair<int,int> svis[k+7];

    int a=0,b=0;

    rep(i , 1 , k+1)
    {
        cin >> chips[i].ff >> chips[i].ss ;
        a=max(chips[i].ff,a),b=max(chips[i].ss,b);
    }
    rep(i , 1 , k+1)
    cin >> svis[i].ff >> svis[i].ss ;

    //cout << chips[1].ff << " " << chips[1].ss << endl;
