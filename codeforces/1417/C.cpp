...
void test(int T)
{
    ...
    veci dist(n+1,INT_MAX);
    rep(i , 1 , n+1)
    {
        ind[i].pb(n+1);
        int maxi=0,lastpos=0;
        rep(j , 0 , ind[i].size())
        {
            maxi=max(maxi,ind[i][j]-lastpos);
            lastpos=ind[i][j];
        }
        dist[maxi]=min(dist[maxi],i);
    }

    rep(i , 2 , n+1) dist[i]=min(dist[i],dist[i-1]);

    rep(i , 1, n+1)
    if(dist[i]==INT_MAX) pf("-1 ");
    else pf("%d ",dist[i]);
