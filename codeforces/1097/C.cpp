 ...
 */
 ...
 using namespace std;
int samne[5*p5],pisone[5*p5],zero=0;

 pii calc(string s)
 {
     int a=0,b=0,i,curr=0;
     rep(i ,0 , s.length())
     {
         if(s[i]=='(') curr++;
         else if(s[i]==')')
 ...
 int main()
 {
     ...
     rep(i , 0 , n)
     {
         ...
         auto p=calc(s);
         if(p.ff==0 and p.ss==0) zero++;
         else if(p.ff and p.ss==0) samne[p.ff]++;
         else if(p.ff==0 and p.ss) pisone[p.ss]++;
     }

     int ans=0,lim=5*p5-1;
     rep(i , 0 , lim)
     {
         if(i) ans+=min(samne[i],pisone[i]);
         else ans=zero>>1;
     }
