...
double find_height(double x,double y,double ax,double ay,double bx,double by)
{
...
x ax bx x
y ay by y
x*ay + ax*by + bx*y - ax*y - bx*ay - x*by
*/
int main()
{
    int n,i;
    ...
    {
        cin >> v[i].ff >> v[i].ss ;
        maxi=max(maxi, find_dis(px,py,v[i].ff,v[i].ss));
    }
    ...
    rep(i , 0 , n)
    {
        mini=min(mini, find_height(px,py,v[i].ff,v[i].ss,v[i+1].ff,v[i+1].ss));
    }

    //error(maxi,mini);
    cout << setprecision(15) << fixed << PI*(maxi-mini) << endl;
