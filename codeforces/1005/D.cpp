...
int solve(int pos,int iscut,int rem)
{
    //error(pos,iscut,rem);
    if(pos==s.length())
    {
    ...
    int &ret=dp[pos][iscut][rem];
    if(ret!=-1) return ret;
    int res=0;
    if(iscut)
    {
        if(s[pos]=='0') res=1+solve(pos+1,1,0);
        else
        {
            int num=s[pos]-'0';
            int cut=(num%3==0)?1:0;
            int a=cut+solve(pos+1,1,0);
            int b=solve(pos+1,0,num%3);
            res=max( a , b );
        }
    }
    else
    {
        //ok;
        int num=s[pos]-'0';
        int cut=(((rem*10)+num)%3==0)?1:0;
        int a=solve(pos+1,0,((rem*10)+num)%3);
        int b=cut+solve(pos+1,1,0);
        res=max( a , b );
    }
