...
void test(int T)
{
    int n,m,i,j,k,maxi=0;
    scin(n);
    veci red(n);
    rep(i , 0 , n) scin(red[i]);
    rep(i , 1 , n) red[i]+=red[i-1];
    scin(m);
    veci blue(m);
    rep(i , 0 , m) scin(blue[i]);
    rep(i , 1 , m) blue[i]+=blue[i-1];

    rep(i , 0 , n)
    {
        rep(j , 0 , m)
        {
            maxi=max(maxi,red[i]+blue[j]);
            maxi=max(maxi,red[i]);
            maxi=max(maxi,blue[j]);
        }
    }
