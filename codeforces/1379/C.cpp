...
int main()
{
    test(t)
    {
        ...
        vecp v(m);
        int i,taken=0,pos;
        rep(i , 0 , m) scin2(v[i].ff,v[i].ss);
        sort(all(v),greater< pair<int,int> >());

        //rep(i , 0 , m) cout << v[i].ff << " " << v[i].ss << endl;

        vecl csum(m+1,0);
        rep(i , 1 , m+1)
        {
            csum[i]=v[i-1].ff+csum[i-1];
        }
        ...
        {
            maxinow=0;
            pair<int,int>p={v[i].ss,0};
            pos=upper_bound(v.rbegin(),v.rend(),p)-v.rbegin();
            taken=min(n,m-pos);
            maxinow+=csum[taken];
            if(v[i].ff<v[i].ss and taken<n) maxinow+=v[i].ff,taken++;
            maxinow+=(1LL*v[i].ss*(n-taken));
            total=max(maxinow,total);
        }
