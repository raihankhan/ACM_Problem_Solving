...
inline bool cmp(  const pair < pair <lli , lli> , lli >  a ,   const pair < pair <lli , lli> , lli > b)
{
...
int main()
{
    ...
    rep(i , 0 , n)
    {
        ...
        ocur[arr[i]]++;
        if(ocur[arr[i]]==1)
        pp[arr[i]].first=i+1,pp[arr[i]].second=i+1;
        else
        {
            pp[arr[i]].second=i+1;
        }
    ...
    rep(i , 0 , p6-2)
    {
        if(ocur[i]>0)
        {
            v.pb(mp(mp(ocur[i],pp[i].second-pp[i].first+1) , i ) );
        }
    }
