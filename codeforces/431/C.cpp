...
inline lli add(lli a, lli b)
{
    a += b;
    return a >= M ? a - M : a;
}
inline lli sub(lli a, lli b)
{
    a -= b;
    return a < 0 ? a + M : a;
}
...
lli dp[107][107][107][2];
lli n,k,d;
lli calc(lli level,lli node,lli sum,bool dfound)
{
    //cout << level << " " << node << " " << sum << " " << dfound << endl;
    lli i,ways=0;
    if(sum>n)
        return 0;
    lli &ret = dp[level][node][sum][dfound];
    if(ret!=-1)
        return ret;
    if(sum==n)
    {
    ...
    {
        if(i>=d)
            ways=add(ways,calc(level+1,i,sum+i,1));
        else
            ways=add(ways,calc(level+1,i,sum+i,dfound));
    }
