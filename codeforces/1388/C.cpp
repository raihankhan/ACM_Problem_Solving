...
///...............................Code Starts Here........................................
int n,m;
int entered[p5],p[p5],h[p5],good[p5],bad[p5];
veci g[p5];
bool dfs(int now,int ances)
{
    bool ret=1;
    int min_good=0;
    entered[now]=p[now];
    for(auto des:g[now])
    {
        ...
        {
            ret=ret & dfs(des,now);
            entered[now]+=entered[des];
            min_good+=good[des];
        }
    }

    good[now]=entered[now]+h[now];          //entered=good+bad; h=good-bad;
    if(good[now]&1) return 0;
    good[now]>>=1;
    if(min_good>good[now]) return 0;
    bad[now]=entered[now]-h[now];
    bad[now]>>=1;

    if(good[now]>entered[now] or bad[now]>entered[now]) return 0;
    return ret;
}
void clean()
{
    ...
    rep(i , 1 , n+1)
    {
        entered[i]=0,p[i]=0,h[i]=0,good[i]=0,bad[i]=0;
        g[i].clear();
    }
...
int main()
{
    ...
    test(t)
    {
        scin2(n,m);
        int i;
        rep(i , 1 , n+1) scin(p[i]);
        rep(i , 1 , n+1) scin(h[i]);
