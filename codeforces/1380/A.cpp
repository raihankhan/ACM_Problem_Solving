...
int main()
{
    test(t)
    {
        ...
        rep(i , 0 , n) scin(v[i]);
        l[0]=v[0],r[n-1]=v[n-1];
        rep(i , 1 , n) l[i]=min(l[i-1],v[i]);
        rev(i , n-2,0) r[i]=min(r[i+1],v[i]);

        rep(i , 1 , n-1)
        {
            if(v[i]!=l[i] and v[i]!=r[i])
            {
                if(v[i]>l[i] and v[i]>r[i])
                {
                    a=l[i],b=v[i],c=r[i];
        ...
        {
            pf("YES\n");
            rep(i , 0 , n) if(v[i]==a) {a=i+1; break;}
            rep(i , 0 , n) if(v[i]==b) {b=i+1; break;}
            rep(i , 0 , n) if(v[i]==c) {c=i+1; break;}
            pf("%d %d %d\n",a,b,c);
        }
    }
