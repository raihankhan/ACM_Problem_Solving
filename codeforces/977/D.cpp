...
void dfs(lli n)
{
    ...
    rep(i , 0 , g[n].size())
    if(!vis[g[n][i]] and mm[g[n][i]]==1)
    dfs(g[n][i]);
}
int main()
{
    lli n,arr[200],i,j,p,a,b;

    scln(n);

    rep(i , 0 , n) scln(arr[i]),mm[arr[i]]=1;
    ...
    {
        a=arr[i]*2;
        if(arr[i]%3==0) b=arr[i]/3;
        else b=-1;

        if(mm[a]==1) g[arr[i]].pb(a) , indeg[a]++;
        if(mm[b]==1) g[arr[i]].pb(b) , indeg[b]++;
    }
