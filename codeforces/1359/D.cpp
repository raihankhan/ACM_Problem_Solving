...
int dp[2][2][p5][31];
int arr[p5],n;
int solve(int s,int e,int i,int maxi)
{
    if(i==n or e==1) return -maxi;
    int &ret=dp[s][e][i][maxi];
    if(ret!=-1) return ret;

    int ret1=-1e9,ret2=-1e9;
    if(!s)
    {
        ret1=arr[i]+solve(1,e,i+1,max(maxi,arr[i]));
        ret2=solve(s,e,i+1,maxi);
    }
    else
    {
        ret1=arr[i]+solve(s,e,i+1,max(maxi,arr[i]));
        ret2=solve(s,1,i+1,max(maxi,arr[i]));
    }
