...
int main()
{
    lli n,i,j,mini,p,d,minia,minib,resta,restb,rest;

    test(t)
    {
        scln(n);
        vecl a(n),b(n);
        minia=LLONG_MAX,minib=LLONG_MAX;
        ...
        rep(i , 0 , n)
        {
            ...
            if(resta==0 and restb==0) continue;
            if(resta==0) mini+=restb;
            if(restb==0) mini+=resta;
            if(resta and restb)
            {
                rest=min(resta,restb);
                mini+=rest;
                mini+=(resta-rest);
                mini+=(restb-rest);
            }
