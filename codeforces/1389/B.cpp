...
lli solve(lli now,lli k,lli z)
{
    if(k==0)
        return v[now];
    lli &ret=dp[k][z],ans=0;
    if(ret!=-1)
        return ret;

    if(now==1)
    {
        ans=v[now]+solve(now+1,k-1,z);
    }
    else
    {
        if(z)
            ans=v[now]+max( solve(now-1,k-1,z-1), solve(now+1,k-1,z) );
        else
            ans=v[now]+solve(now+1,k-1,z);
    }
