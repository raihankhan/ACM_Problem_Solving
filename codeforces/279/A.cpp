...
void test(int T)
{
    int x,y,nowx=0,nowy=0,i;
    scin2(x,y);
    int turn=0;
    int mod=0,mov=1;
    while(1)
    {
        if(nowx==x and nowy==y) break;
        if(mod==0) rep(i , 0 , mov) { nowx++; if(nowx==x and nowy==y) break; }
        else if(mod==1)  rep(i , 0 , mov) { nowy++; if(nowx==x and nowy==y) break; }
        else if(mod==2)  rep(i , 0 , mov) { nowx--; if(nowx==x and nowy==y) break; }
        else if(mod==3)  rep(i , 0 , mov) { nowy--; if(nowx==x and nowy==y) break; }
        if(mod==1 or mod==3) mov++;
        mod++;
        mod%=4;
        turn++;
    }
