...
void test(int T)
{
    lli p,f,cnts,cntw,s,w,rems,remw,temp,ans=0,takena,takenb,takenc,takend;
    ...
    {
        if(takena*s>p) break;
        takenb= min((p-(takena*s))/w , cntw );
        takenc= min( f/s , cnts-takena );
        takend= min( (f-(takenc*s))/w , cntw-takenb);

        ans=max(ans , takena + takenb + takenc + takend);
    }
