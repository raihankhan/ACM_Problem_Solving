...
using namespace std;
vector<pair<lli,lli>>x[1007];
vector<pair<lli,lli>>y[1007];
...
void dfs(lli a,lli b)
{
    ...
    rep(i , 0 , x[a].size())
    {
        if(!visited[x[a][i].ff][x[a][i].ss])
            dfs(x[a][i].ff,x[a][i].ss);
    }

    rep(i , 0 , y[b].size())
    {
        if(!visited[y[b][i].ff][y[b][i].ss])
            dfs(y[b][i].ff,y[b][i].ss);
    }
