...
int main()
{
    ...
    string s;
    cin >> s;
    lli B=0,S=0,C=0,len=s.length(),i;
    rep(i, 0,  len)
    {
        if(s[i]=='B') B++;
        if(s[i]=='C') C++;
    ...
    lli low=1,high=1LL*p6*p6,mid,left;
    lli pc=(bc*B)+(sc*S)+(cc*C);
    //cout << pc << endl;
    ...
    while(low <= high)
    {
        ...
        lli ex=0;
        if(B) ex+=(bc*min(bs,B*mid));
        if(S) ex+=(sc*min(ss,S*mid));
        if(C) ex+=(cc*min(cs,C*mid));
       // cout << ex << endl;
