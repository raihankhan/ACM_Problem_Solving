...
int dp[(1<<24)+7],n;
int x[26],y[26];
vector<int>path[(1<<24)+7];
bool Is_Set(int mask,int pos) { return mask&(1<<pos); }
int Set(int mask,int pos) { return mask|(1<<pos); }
int Set2(int mask,int pos1,int pos2) { return mask|(1<<pos1)|(1<<pos2); }
int dis(int src,int des)
{
    src++,des++;
    return (x[src]-x[des])*(x[src]-x[des])+(y[src]-y[des])*(y[src]-y[des]);
}
int solve(int mask)
{
    if(__builtin_popcount(mask)==n) return 0; // all bags collected
    if(dp[mask]!=-1) return dp[mask];
    int i,j;
    int &ret=dp[mask],temp;
    ret=1<<30;
    rep(i , 0 , n)
    {
        if(!Is_Set(mask,i))
        {
            rep(j , 0 , n)
            {
                ...
                if(!Is_Set(mask,j))
                {
                    temp=solve(Set2(mask,i,j))+dis(-1,i)+dis(i,j)+dis(j,-1);  // 0-i-j-0
                    if(ret>temp)
                    {
                        ret=temp;
                        veci v;
                        v.pb(-1),v.pb(i),v.pb(j);
                        path[mask]=v;
                    }
                }
            }
            temp=solve(Set(mask,i))+dis(-1,i)+dis(i,-1); // 0-i-0
            if(ret>temp)
            {
                ret=temp;
                veci v;
            ...
            break;  // each time make one choice and come back to handbag
        }
    }
...
{
    if(__builtin_popcount(mask)==n) { pf("0\n"); return;}
    int i,newmask=mask;
    rep(i , 0 , path[mask].size())
    {
        pf("%d ",path[mask][i]+1);
        if(path[mask][i]!=-1)
        newmask=Set(newmask,path[mask][i]);
    }
    print_path(newmask);
}
int main()
{
    scin2(x[0],y[0]); // handbag's location
    scin(n);
    int i;
    rep(i , 1 , n+1)
    scin2(x[i],y[i]);
