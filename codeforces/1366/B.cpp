...
void test(int T)
{
    int n,x,m,i,j,l,r,ml=INT_MAX,mr=INT_MIN,p=0;
    cin >> n >> x >> m ;

    while(m--)
    {
        scin2(l,r);
        if(ml==INT_MAX and mr==INT_MIN)
        {
           if(l<=x and r>=x)
                ml=min(ml,l),mr=max(mr,r),p=1;
        }
        else
        {
            if((l<=ml and r>=ml) or (l<=mr and r>=mr))
                 ml=min(ml,l),mr=max(mr,r);
        }
    }

    if(p) cout << mr-ml+1 << endl;
    else cout << "1" << endl;
