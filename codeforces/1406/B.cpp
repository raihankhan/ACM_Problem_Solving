...
lli solve(lli id,lli taken,lli mul)
{
    if(taken==5) return mul;
    if(id==m) return LLONG_MIN;
    return max( solve(id+1,taken,mul),solve(id+1,taken+1,mul*selected[id]) );
}
...
void test(int T)
{
    ...
    m=0;
    rep(i , 0 , 5) selected[m]=v[i],m++;

    for(i=n-1; i>=n-5 and i>=5 ; i--)
    selected[m]=v[i],m++;
