...
public:
    int range,l,r;
    segment(int range, int l,int r) : range(range),l(l),r(r)
    {
    }
};
...
int main()
{
    int t,n,i,j,k,p,l,r,mid,mov,now,range;
    ...
    while(t--)
    {
        ...
        priority_queue<segment>pp;
        l=0,r=n-1,mov=0;

        pp.push( segment(r-l+1,l,r) );
        ...
        while(mov<n)
        {
            ...
            if(range&1) mid=(l+r)/2;
            else mid=(l+r-1)/2;

            arr[mid]=mov;
            if(l==r) continue;

            pp.push( segment(mid-l,l,mid-1) );
            pp.push( segment(r-mid,mid+1,r) );
        }
