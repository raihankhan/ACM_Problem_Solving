...
void test(int T)
{
    ...
    rep(i , 0 , n) scln2(b[i],d[i]);

    sort(all(b));
    sort(all(d));

    lli bi=0,di=0,alive=0,maxi=LLONG_MAX,k=0;

    while(bi<n or di<n)
    {
        if(bi<n and di<n and b[bi]<d[di])
        {
            alive++;

            if(alive==k) maxi=min(b[bi],maxi);
            else if(alive>k) k=alive,maxi=b[bi];

            bi++;
        }
        else if(di<n and bi<n and d[di]<b[bi])
        {
            alive--;
            di++;
        }
