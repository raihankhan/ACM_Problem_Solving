...
public:
    int vol,tea,id;
    node(int a,int b,int c) { vol=a,tea=b,id=c; }
};

bool cmp(const node &a,const node &b)
{
    if(a.vol==b.vol) return a.tea>b.tea;
    return a.vol>b.vol;
}
void test(int T)
{
    ...
    rep(i , 0 , n)
    {
        need=v[i]&1?(v[i]>>1)+1:v[i]>>1;
        w-=need;
        cups.pb(node(v[i],need,i));
    }
    ...
    rep(i , 0 , n-1)
    {
        rep(j , i+1 , n)
        {
            if(cups[i].vol>cups[j].vol and cups[i].tea<cups[j].tea)
            {
                need=cups[j].tea-cups[i].tea+1;
                cups[i].tea+need;
                w-=need;
            }
        }
    ...
    rep(i , 0 , n)
    {
        if(cups[i].tea>cups[i].vol)
        {
            pf("-1\n"); return;
        }
        if(w)
        {
            need=min(w,cups[i].vol-cups[i].tea);
            cups[i].tea+=need;
            w-=need;
        }

        v[cups[i].id]=cups[i].tea;
    }
