...
///...............................Code Starts Here........................................
lli dp[207][207][207];
lli red[207],green[207],blue[207];
lli r,g,b;
bool term(lli p1,lli p2,lli p3)
{
    if(p1==r and p2==g) return 1;
    if(p2==g and p3==b) return 1;
    if(p3==b and p1==r) return 1;
    return 0;
}
lli solve(lli p1,lli p2,lli p3)
{
    if(term(p1,p2,p3)) return 0;
    lli take12=0,take23=0,take31=0,take_none=0, &ret=dp[p1][p2][p3];

    if(ret!=-1) return ret;

    take_none=max(take_none,p3+1<b?solve(p1,p2,p3+1):0);
    take_none=max(take_none, p1+1<r?solve(p1+1,p2,p3):0);
    take_none=max(take_none, p2+1<g?solve(p1,p2+1,p3):0);

    if(p1<r and p2<g) take12=(red[p1]*green[p2])+ solve(p1+1,p2+1,p3);
    if(p2<g and p3<b) take23=(green[p2]*blue[p3])+ solve(p1,p2+1,p3+1);
    if(p3<b and p1<r) take31=(blue[p3]*red[p1])+ solve(p1+1,p2,p3+1);

    return ret=max(max(take_none,take12) ,max(take23,take31) );
}
int main()
{
    IOS;
