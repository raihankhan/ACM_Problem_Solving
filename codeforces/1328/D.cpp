...
int main()
{
    int t,i,j,p,maxi,n,pos;
    scin(t);
    while(t--)
    {
        scin(n);
        veci v(n<<1),color(n+2,0),repeated;

        rep(i, 1, n+1) scin(v[i]);
        ...
        // rep(i , 1 , n+1) cout << color[i] << " " ; pfn;

        if(repeated.size()==n-1)
        {
            pf("1\n");
            rep(i, 1, n+1) i==n?pf("1\n"):pf("1 ");
        }
        else if(v[n]!=v[1] and color[1]==color[n])
        {
            if(repeated.size()==0)
            {
                color[n]=3;
                pf("3\n");
                rep(i, 1, n+1) i==n?pf("%d\n",color[i]):pf("%d ",color[i]);
            }
            else
            {
                pos=repeated[repeated.size()-1];
                rep(i, pos, n+1)
                {
