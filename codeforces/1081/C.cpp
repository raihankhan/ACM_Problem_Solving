...
using namespace std;
lli n,m,k,dp[2007][2007];
lli solve(lli brick,lli taken)
{
    ...
    lli &ret = dp[brick][taken];
    if(ret!=-1LL) return ret;
    if(!brick)
    {
        p1=add(p1,mul(m,solve(brick+1,taken)));
        return ret=p1;
    }

    p1 = add(p1,solve(brick+1,taken));
    p2 = add(p2,mul((m-1),solve(brick+1,taken+1)));

    return ret=add(p1,p2);
