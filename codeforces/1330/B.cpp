...
int main()
{
    ...
    rep(t , 1 , k+1)
    {
        ...
        rep(i , 1 , n+1)
        {
            ...
            {
                rgt[arr[n-i+1]]=t;
                cumr[n-i+1]=arr[n-i+1]+cumr[n-i+2];
            }
            else cumr[n-i+1]=maxi;
        }

        //rep(i , 1 , n+1) cout << cuml[i] << " "; pfn;
        //rep(i , 1 , n+1) cout << cumr[i] << " "; pfn;

        vecp v;

        rep(i , 1 , n)
        {
            if(cuml[i]==(i*(i+1))/2 and cumr[i+1]==((n-i)*(n-i+1))/2 )
            v.pb({i,n-i});
        }
