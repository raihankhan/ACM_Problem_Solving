...
int main()
{
    ...
    for(auto f:fir)
    {
        //error(f.ff,f.ss,sec[f.ff],coin[f.ff]);
        if(f.ss>=need) mini=0;
        else
        {
            if( (sec[f.ff]-coin[f.ff])>=(need-f.ss))
                mini=min(mini,need-f.ss);
        }
    }
    for(auto b:sec)
    {
        if(fir[b.ff]) continue;
        if(b.ss>=need) mini=min(mini,need);
    }
    if(mini==M) mini=-1;
    pf("%d\n",mini);
