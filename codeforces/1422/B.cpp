...
void test(int T)
{
    ...
    rep(i , 1 , n+1)
    {
        rep(j , 1 , m+1)
        {
            if(arr[i][j]!=arr[i][m-j+1] or arr[i][j]!=arr[n-i+1][j])
            {
                k=LLONG_MAX,res=0;
                res=abs(arr[n-i+1][j]-arr[i][j])+abs(arr[n-i+1][j]-arr[i][m-j+1]);
                if(res<=k) k=res,id=1;
                res=abs(arr[i][j]-arr[n-i+1][j])+abs(arr[i][j]-arr[i][m-j+1]);
                if(res<=k) k=res,id=2;
                res=abs(arr[i][m-j+1]-arr[n-i+1][j])+abs(arr[i][m-j+1]-arr[i][j]);
                if(res<=k) k=res,id=3;

                if(id==1) arr[i][j]=arr[n-i+1][j],arr[i][m-j+1]=arr[n-i+1][j];
                if(id==2) arr[n-i+1][j]=arr[i][j],arr[i][m-j+1]=arr[i][j];
                if(id==3) arr[i][j]=arr[i][m-j+1],arr[n-i+1][j]=arr[i][m-j+1];
