...
///...............................Code Starts Here........................................
int status[402][25][25]; //t*coder(n)*seat(m)
int wheel[402][25]; //t*m
  int main()
{
    int i,j,k,t,n,m,bottom,seat,coder,rides;
      scanf("%d",&t);
      for(k=1; k<=t; k++)
    {
        scanf("%d %d",&n,&m);
          ...
          while(rides)
        {
            for(seat=1;seat<=m;seat++)
            {
                if(wheel[k][seat])
                {
                    ...
                    q.push(coder);
                    wheel[k][seat]=0;
                    status[k][seat][coder]=1;
                    rides--;
                }
                if(!rides) { bottom++; break; }
                  while(!q.empty())
                {
                    coder=q.front();
                    if(!status[k][seat][coder])
                    {
                        wheel[k][seat]=coder;
                        q.pop();
                        break;
