...
lli ways;
int lcs_dp[32][32];
lli ways_dp[32][32][62];
int lcs(int i,int j)
{
    if(i==alen or j==blen) return 0;
    int &ret=lcs_dp[i][j];
    if(ret!=-1) return ret;
    ret=0;
    if(a[i]==b[j])
  ...
  lli cal_ways(int i,int j,int l)
{
    if(i==alen)
    {
        if(l+blen-j==len) return 1;
        else return 0;
    }
    if(j==blen)
    {
        if(l+alen-i==len) return 1;
        else return 0;
    }
    lli &ret= ways_dp[i][j][l];
    if(ret!=-1) return ret;
    if(a[i]==b[j])
         return ret=cal_ways(i+1,j+1,l+1);
    else return ret=cal_ways(i+1,j,l+1)+cal_ways(i,j+1,l+1);
}
int main()
{
    int k,t;
    ...
    for(k=1;k<=t;k++)
    {
        ...
        mem(lcs_dp,-1);
        mem(ways_dp,-1);
        len=alen+blen-lcs(0,0);
        ways=cal_ways(0,0,0);
        cout << "Case " << k << ": " << len << " " << ways << endl;
    }
        return 0;
}