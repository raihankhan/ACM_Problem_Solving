  ...
  lli cal_perm(int len)
{
  ...
  string solve(int pos,int rem,int len)
{
    int i;
    lli used;
    string ret="";
    ...
    for(i=0;i<26;i++)
    {
        ...
        {
            cnt[i]--;
            used=cal_perm(len-pos-1);
              if(used>=rem)
            {
                ret=char('a'+i)+solve(pos+1,rem,len);
                return ret;
            }
            cnt[i]++;
            rem-=used;
  ...
  int main()
{
      ...
      for(k=1;k<=t;k++)
    {
        ...
        for(i=0;i<s.length();i++)
            cnt[s[i]-'a']++;
          if(cal_perm(s.length())<n)
        {
            pf("Case %d: Impossible\n",k);
            continue;
        }
          string nth_perm=solve(0,n,s.length());
          cout << "Case " << k << ": " << nth_perm ;
        pfn;
    }
        return 0;
}
