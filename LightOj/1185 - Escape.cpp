  ...
  void clean()
{
    ...
    {
        g[i].clear();
        visited[0][i]=0,visited[1][i]=0;
    }
}
int main()
{
    int t,i,j,k,n,m,u,v,ans=0;
      scin(t);
      rep(i, 1, t+1)
    {
        scin2(n,m);
          ...
          while(!q.empty())
        {
              curr=q.front();
            q.pop();
              rep(j, 0, g[curr.ss].size())
            {
                if(curr.ff==1)
                {
                    if(!visited[0][g[curr.ss][j]])
                    {
                        visited[0][g[curr.ss][j]]=1;
                        q.push({0,g[curr.ss][j]});
                    }
                }
                else
                {
                    if(!visited[1][g[curr.ss][j]])
                    {
                        visited[1][g[curr.ss][j]]=1;
                        q.push({1,g[curr.ss][j]});
                    }
                }
            }
        }
          rep(j, 1, n+1) ans+=visited[1][j];
        pf("Case %d: %d\n",i,ans);
          clean();
    }
      #ifdef HOME
