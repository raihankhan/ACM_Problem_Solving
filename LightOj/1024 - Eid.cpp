...
string multiply(string a,lli b)
{
    lli carry=0,i;
...
int main()
{
    ...
    for(k=1; k<=t; k++)
    {
        ...
        {
            scanf("%d",&days[i]);
              for(j=0; prime[j]*prime[j]<=days[i]; j++)
            {
                p=1LL;
                while(days[i]%prime[j]==0)
                {
                    p*=prime[j];
                    days[i]/=prime[j];
                }
            ...
            if(days[i]>1)
            {
                j=lower_bound(all(prime),days[i])-prime.begin();
                fact[j]=max(fact[j],days[i]*1LL);
            }
        }
          string lcm="1";
