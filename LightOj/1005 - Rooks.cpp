...
*/
  using namespace std;
  lli dp[40][40];
  lli fdp(lli board , lli rooknum)
{
    if(board<rooknum) return dp[board][rooknum]=0;
    if(rooknum==0) return dp[board][rooknum]=1;
    if(dp[board][rooknum]>-1) return dp[board][rooknum];
    return dp[board][rooknum]= (board*board*fdp(board-1,rooknum-1))/rooknum;
}
  int main()
{
   lli t,n,k,ans,i;
