...
int solve(int mask)
{
    ...
    int shots=(1<<30),damage,kill_cost,i,j;
    rep(i , 0 , n)
    {
        if(Is_Set(mask,i))
        {
            rep(j , 0 , n)
            {
                if(v[i][j]!='0' and i!=j and !Is_Set(mask,j))
                {
                    damage=v[i][j]-'0';
                    if(health[j]%damage==0) kill_cost=health[j]/damage;
                    else kill_cost=(health[j]/damage)+1;
                    shots=min(shots,kill_cost+solve(Set(mask,j)));
                }
            }
        }
        else
        {
            shots=min(shots,health[i]+solve(Set(mask,i)));
        }
    }
    return dp[mask]=shots;
}
  int main()
{
    int t,i,j,k;
